...
<div id="result-options" class="panel-wrap{if !$show_filters} hidden-xs{/if}">
    <div class="row">
        <div class="col-xs-12 displayoptions form-inline text-center {*CUSTOM}hidden-xs{//CUSTOM*}">
            {block name="productlist-result-options-sort"}
			{*CUSTOM}{if $show_filters}
                <h3 class="title_cstm" data-toggle="collapse" href="#filter-collapsible" aria-expanded="false" aria-controls="filter-collapsible">
                    <span class="fa fa-filter"> {lang key='filterBy' section='custom'}<span class="caret"></span></span>
                </h3>
			{/if}{//Custom*}

            {*CUSTOM_SG if isset($oErweiterteDarstellung) && isset($Einstellungen.artikeluebersicht.artikeluebersicht_erw_darstellung) && $Einstellungen.artikeluebersicht.artikeluebersicht_erw_darstellung === 'Y'}
                <div class="btn-group">
    ...
    {if $show_filters}
        <div id="filter-collapsible" class=" top10">
            ...
            <div id="navbar-filter" class="panel-body">
                <div class="form-inline">
                    ...
                    {if $Einstellungen.navigationsfilter.merkmalfilter_verwenden === 'content' && $Suchergebnisse->MerkmalFilter|@count > 0 && $Suchergebnisse->Artikel->elemente|@count > 0}
                        ...
                        {*CUSTOM*}
                        {customHauptmerkmale MerkmalFilter=$Suchergebnisse->MerkmalFilter Hauptmerkmale=$oNavigationsinfo->oKategorie->KategorieAttribute.hauptmerkmale return=MerkmalFilter_arr}
                        {*//CUSTOM*}
                        {*CUSTOM*}
                        {if isset($MerkmalFilter_arr->haupt)}
                            {assign var=MerkmalFilter value=$MerkmalFilter_arr->haupt}
                        {else}
                            {assign var=MerkmalFilter value=$Suchergebnisse->MerkmalFilter}
                        {/if}
                        {*//CUSTOM*}
                        {*CUSTOM foreach name=merkmalfilter from=$Suchergebnisse->MerkmalFilter item=Merkmal //CUSTOM*}
                        {*CUSTOM*}{foreach name=merkmalfilter from=$MerkmalFilter item=Merkmal}{*//CUSTOM*}
                        ...
                        {/foreach}
                        {*CUSTOM*}
                        {if isset($MerkmalFilter_arr->haupt) && !empty($MerkmalFilter_arr->haupt)}
                            <div id="extend-merkmale" class="form-group dropdown">
                                <div class="filter-type-characteristic">
                                    <a class="btn btn-default form-control" data-toggle="collapse" href="#filter-collapsible-extend" aria-expanded="false" aria-controls="filter-collapsible-extend">
                                        Auswahl verfeinern <span class="caret"></span>
                                    </a>
                                </div>
                                <div id="filter-collapsible-extend" class="collapse">
                                    <div class="wrapper">
                                        {foreach from=$MerkmalFilter_arr->weitere item=Merkmal}
                                            <div class="form-group dropdown filter-type-characteristic">
                                                <a class="" data-toggle="collapse" href="#extend-kM{$Merkmal->kMerkmal}">
                                                    {$Merkmal->cName} <i class="fa fa-chevron-down"></i>
                                                </a>
                                                <div id="extend-kM{$Merkmal->kMerkmal}" class="collapse">
                                                    {include file='snippets/filter/characteristic.tpl' role="menu"}
                                                </div>
                                            </div>
                                        {/foreach}
                                    </div>
                    ...
                    {/if}
			{*CUSTOM_SG FILTER IN TOGGLE PLATZIERT*}					
				{*CUSTOM}<div class="form-group hidden-xs">
					<select name="Sortierung" onchange="$('#improve_search').submit();" class="btn btn-default dropdown-toggle form-control">
						{if !isset($Suchergebnisse->Sortierung) || !$Suchergebnisse->Sortierung}
							<option class="sort_cstm" value="0">{lang key="sorting" section="productOverview"}</option>{/if}
						<option class="sort_cstm" value="100" {if isset($smarty.session.Usersortierung) && isset($Sort) && $smarty.session.Usersortierung==$Sort->value}selected="selected"{/if}>{lang key="standard" section="global"}</option>
						{foreach name=sortierliste from=$Sortierliste item=Sort}
							<option class="sort_cstm" value="{$Sort->value}" {if $smarty.session.Usersortierung==$Sort->value}selected="selected"{/if}>{$Sort->angezeigterName}</option>
						{/foreach}
					</select>
				</div>{//CUSTOM*}
				{*CUSTOM Items per Page}
				<div class="form-group hidden-xs">
					<select name="af" onchange="$('#improve_search').submit();" class="btn btn-default dropdown-toggle form-control">
				...
				{//CUSTOM*}
				{*CUSTOM if isset($oErweiterteDarstellung) && isset($Einstellungen.artikeluebersicht.artikeluebersicht_erw_darstellung) && $Einstellungen.artikeluebersicht.artikeluebersicht_erw_darstellung === 'Y'}
