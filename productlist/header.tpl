...
{*CUSTOM_SG IMPROVED_SEARCH TOPPLACED*}
...
{if isset($oNavigationsinfo->cName) && $oNavigationsinfo->cName !== '' || isset($oNavigationsinfo->cBildURL) && !empty($oNavigationsinfo->cBildURL)}
	{*CUSTOM_SG NEW TITEL_POS ON XS*}
		<div class="title visible-xs text-center">{if $oNavigationsinfo->cName}<h1>{$oNavigationsinfo->cName}</h1>{/if}</div>
	{*//CUSTOM_SG NEW TITEL_POS ON XS*}    
	<div class="desc clearfix">
        ...
        {/if}
		{*CUSTOM_SG NEW TITEL_POS HIDDEN ON XS*}
			<div class="title hidden-xs">{if $oNavigationsinfo->cName}<h1>{$oNavigationsinfo->cName}</h1>{/if}</div>
		{*//CUSTOM_SG NEW TITEL_POS HIDDEN ON XS*}
